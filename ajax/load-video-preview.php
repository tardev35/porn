 ...
 * @return void
 */
function ftt_load_video_preview() {
	check_ajax_referer( 'ajax-nonce', 'nonce' );
	if ( ! isset( $_POST['post_id'] ) ) {
		wp_send_json_error( array( 'message' => 'post_id parameter is missing' ) );
	}
	try {
		wp_send_json_success( ftt_get_video_preview( intval( $_POST['post_id'] ) ) );
	} catch ( \Exception $exception ) {
		wp_send_json_error( array( 'message' => $exception->getMessage() ) );
	}
	wp_die();
}
add_action( 'wp_ajax_ftt_load_video_preview', 'ftt_load_video_preview' );
add_action( 'wp_ajax_nopriv_ftt_load_video_preview', 'ftt_load_video_preview' );
